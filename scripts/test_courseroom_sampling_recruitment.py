#!/usr/bin/env python3
"""Validate the standalone courseroom sampling/recruitment download."""
...
MD = ROOT / "docs" / "courseroom-sampling-recruitment.md"
DOCX = ROOT / "downloads" / "WALKER_Project_Plan_Sampling_and_Recruitment.docx"
...
def main() -> int:
    ...
    if SOP.exists() and "## Courseroom paste" in SOP.read_text(encoding="utf-8"):
        errors.append("SOP was modified with courseroom paste blocks")
    if errors:
        print("Courseroom sampling/recruitment validation FAILED")
        for err in errors:
            print(f"  - {err}")
        return 1
    print("Courseroom sampling/recruitment validation OK")
